...
**Solution 1 - RECOMMANDÉE (débloquer les scripts + politique)** :
```powershell
# Débloquer les scripts (une seule fois)
Unblock-File -Path .\scripts\*.ps1

# Autoriser l'exécution de scripts (permanent)
...
**Solution 2 - RAPIDE (politique Bypass permanent)** :
```powershell
# Bypass complet (pas de vérification de signature)
...
**Solution 3 - Temporaire (juste cette session)** :
```powershell
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process
.\scripts\fusionne.ps1 claude/fix-safe-directory-warning-01MdoVv6j7to7ph4yW1cw7wf
```

**Solution 4 - Exécution unique** :
```powershell
